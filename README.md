...
In order to do that, it is necessary to create a `doc` instance of `jsPDF()` class. Then it is declared variables `imgCheck` and `imgX` assigning them to **base64** code in which are encoded icons. Afterward, variables are declared as coordinates for pdf elements and in the next step `container` handler (the current list) is passed through the nested loop, to place persons data in a proper sequence.
